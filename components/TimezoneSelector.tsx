  ...
  if (!isHydrated) {
    return (
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <select disabled className="w-full sm:flex-1 px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
          <option>Loading...</option>
        </select>
        <div className="w-full sm:w-auto px-4 py-2 text-sm font-semibold text-white bg-[#1E40AF] rounded-lg opacity-50 text-center">Save</div>
      </div>
    );
  }
  ...
  return (
    <form className="flex flex-col sm:flex-row sm:items-center gap-4">
      ...
      <select
        ...
        className="w-full sm:flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 bg-white
